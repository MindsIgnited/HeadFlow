...
class JModuleHelper
{
	...
	function &getModules($position)
	{
		...
		if(count($result) == 0) {
                        // FIXME: remove global.. Infact could proprobably just remove this class all together and set the tp parameter on the request.
			if(JRequest::getBool('tp')) {
				$result[0] = JModuleHelper::getModule( 'mod_'.$position );
				$result[0]->title = $position;
				$result[0]->content = $position;
				$result[0]->position = $position;
			}else if(defined('HEADFLOW_ACTIVE')){
                            	$result[0] = JModuleHelper::getModule( 'mod_'.$position );
				$result[0]->title = '';
				$result[0]->content = '';
				$result[0]->position = $position;
	...
	function renderModule($module, $attribs = array())
	{
		...
		return '<div class="headFlowWidgetContainer" >' . $module->content . '</div>';
	}
